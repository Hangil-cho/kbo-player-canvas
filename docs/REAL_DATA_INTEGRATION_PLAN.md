...
- Statiz 지표는 계산 방식이 자체적일 수 있으므로 직접 재계산하지 않고 출처 값을 표시합니다.
- 자동 수집이 불안정하면 처음에는 CSV 수동 입력 템플릿을 만들어도 됩니다.
...
그 다음에 KBO 공식 기록 상세 수집을 붙이고, 마지막에 Statiz WAR/RAA를 붙이는 순서가 가장 안전합니다.

## 사용자가 정하면 좋은 것

- 실제 데이터 우선 적용 선수 1명
- KBO 공식 기록과 Statiz 중 어느 쪽을 먼저 신뢰 기준으로 둘지
- 자동 수집이 안 되는 지표를 수동 CSV로 관리해도 되는지
- WAR 기준을 Statiz로 고정할지, 다른 출처도 같이 비교할지
